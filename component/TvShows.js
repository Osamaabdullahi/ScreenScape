...
export default function Tvshows() {
  ...
  const fetchMoreData = () => {
    ...
    // Load the next batch of movies
    setMovies((prevMovies) => [
      ...prevMovies,
      ...allMovies.slice(prevMovies.length, prevMovies.length + visibleCount),
    ]);
  };
      ...
      <main className="pt-20">
        <section className="container mx-auto px-6 py-16">
          ...
          <InfiniteScroll
            ...
            endMessage={<p>No more shows to display</p>}
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
              {Movies.map((movie, index) => (
                <Link href={`Tvshows/watch/?id=${movie.id}`}>
                  <div
                    key={movie.id}
